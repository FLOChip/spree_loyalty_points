  ...
  <% if !@order.loyalty_points_used? && @order.eligible_for_loyalty_points?(@order.item_total) %>
    <%= f.field_container :loyalty_points do %>
      ...
      <% else %>
        ...
        <br /> User's Mini Points Balance: <%= @order.user.loyalty_points_balance %> <br /> Net Mini Points Credited for this Order: <%= @order.loyalty_points_total %>
      <% end %>
    <% end %>
  <% elsif @order.loyalty_points_used? %>
    <%= f.field_container :loyalty_points do %>
      ...
      <% else %>
        ...
        <br /> User's Mini Points Balance: <%= @order.user.loyalty_points_balance %> <br /> Net Mini Points Debited for this Order: <%= -@order.loyalty_points_total %>
      <% end %>
    <% end %>
  <% end %>
  ")
