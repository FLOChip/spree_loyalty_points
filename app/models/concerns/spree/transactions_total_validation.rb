    ...
    extend ActiveSupport::Concern
      ...
      def validate_transactions_total_range(trans_type, relation)
        ...
        if first_transaction.transaction_type == "Debit"
          errors.add :base, "Mini Points Net Debit Total should be in the range [0, #{ first_transaction.loyalty_points }]" if net_transactions_total > 0 || net_transactions_total < -first_transaction.loyalty_points
        else
          errors.add :base, "Mini Points Net Credit Total should be in the range [0, #{ first_transaction.loyalty_points }]" if net_transactions_total < 0 || net_transactions_total > first_transaction.loyalty_points
        end
      end
